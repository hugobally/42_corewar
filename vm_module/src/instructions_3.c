...
t_errors			ft_sti(t_core *core, t_process *process)
{
	...
	if (p2 == REG)
	{
		if (p3 == REG)
			...
			write_val(core, get_pc((process->regs[params.p2 - 1] + process->regs[params.p3 - 1]) % IDX_MOD), 4, process->regs[params.p1 - 1]);
		if (p3 == DIR)
			//*((int32_t*)&(core->arena[get_pc((process->regs[params.p2 - 1] + params.p3) % IDX_MOD)])) = process->regs[params.p1 - 1];
			write_val(core, get_pc((process->regs[params.p2 - 1] + params.p3) % IDX_MOD), 4, process->regs[params.p1 - 1]);
	}
	if (p2 == DIR)
	{
		if (p3 == REG)
			//*((int32_t*)&(core->arena[get_pc((params.p2 + process->regs[params.p3 - 1]) % IDX_MOD)])) = process->regs[params.p1 - 1];
			write_val(core, get_pc((params.p2 + process->regs[params.p3 - 1]) % IDX_MOD), 4, process->regs[params.p1 - 1]);
		if (p3 == DIR)
			//*((int32_t*)&(core->arena[get_pc((params.p2 + params.p3) % IDX_MOD)])) = process->regs[params.p1 - 1];
			write_val(core, get_pc((params.p2 + params.p3) % IDX_MOD), 4, process->regs[params.p1 - 1]);
	}
	if (p2 == IND)
	{
		ft_printf("p2 is IND\n");
