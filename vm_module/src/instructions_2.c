...
t_errors	ft_xor(t_core *core, t_process *process)
{
	...
	p1 = 0;
	p2 = 0;
	if ((ret = ft_reg(process, &pa[0], &pa[1], &pa[2])) != ok)
		return (ok);
	if (pa[0] == REG)
		...
		p1 = core->arena[get_pc(process->pc + (process->params.p1 % IDX_MOD))];
	if (pa[1] == REG)
		p2 = process->regs[process->params.p2 - 1];
	else if (pa[1] == DIR)
		p2 = process->params.p2;
	else if (pa[1] == IND)
		p2 = core->arena[get_pc(process->pc + (process->params.p2 % IDX_MOD))];
